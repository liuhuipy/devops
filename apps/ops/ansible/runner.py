...
sys.path.append('/tmp/ansible-demo/')
...
from ops.ansible.callback import AdHocResultCallback, PlaybookResultCallBack, \
...
def test_run():
    ...
    task_tuple = (('shell', 'ifconfig eno16777736'),)  ##例子，调用普通的模块命令
    hoc = AdHocRunner(hosts=assets)
    hoc.results_callback = CommandResultCallback()
    ret = hoc.run(task_tuple)
    print(ret)

    # runner = AdHocRunner(assets, forks=5)
    # result = runner.run(task_tuple=[('script', '/Users/liuhui/PycharmProjects/devops/test.sh')])
    # print(result)
    ...
    # play = PlayBookRunner(assets, playbook_path='/root/inventory/inventory.yml')  ##yml
    """
    # /tmp/some.yml
    ---
